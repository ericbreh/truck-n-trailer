...
class AutoStateView(QWidget):
    ...
    def _bounds(self) -> tuple[float, float, float, float]:
        ...
        y_min = min(ys)
        y_max = max(ys)
        # Keep enough room for the full truck+trailer geometry around tracked points.
    ...
    def paintEvent(self, event) -> None:  # type: ignore[override]
        ...
        margin = 16.0
        draw_w = max(1.0, self.width() - 2.0 * margin)
        draw_h = max(1.0, self.height() - 2.0 * margin)
        scale = min(draw_w / w, draw_h / h)

        def to_px(x: float, y: float) -> tuple[float, float]:
            px = margin + (x - x_min) * scale
            py = self.height() - (margin + (y - y_min) * scale)
            return px, py
        ...
        # Goal marker
        if self.goal_q is not None:
            gx, gy = to_px(float(self.goal_q[0]), float(self.goal_q[1]))
            painter.setPen(QPen(QColor("#dc2626"), 2))
            painter.drawLine(int(gx - 8), int(gy - 8), int(gx + 8), int(gy + 8))
            painter.drawLine(int(gx - 8), int(gy + 8), int(gx + 8), int(gy - 8))

        # Truck + trailer pose
        x, y, theta_t, theta_l = [float(v) for v in self.current_q[:4]]
        x_front = x + self.truck_len_cm * math.cos(theta_t)
        y_front = y + self.truck_len_cm * math.sin(theta_t)
        x_trailer = x - self.trailer_len_cm * math.cos(theta_l)
        y_trailer = y - self.trailer_len_cm * math.sin(theta_l)
        hx, hy = to_px(x, y)
        fx, fy = to_px(x_front, y_front)
        tx, ty = to_px(x_trailer, y_trailer)

        painter.setPen(QPen(QColor("#111827"), 4))
        painter.drawLine(int(hx), int(hy), int(fx), int(fy))
        painter.setPen(QPen(QColor("#2563eb"), 4))
        painter.drawLine(int(hx), int(hy), int(tx), int(ty))
...
class TruckControlGui(QWidget):
    def __init__(self, args: argparse.Namespace):
        ...
        self.auto_status_label = QLabel("MPC: Idle")
        self.auto_state_view = AutoStateView(
            truck_len_cm=AUTO_TRUCK_LENGTH_CM,
            trailer_len_cm=AUTO_TRAILER_LENGTH_CM,
        )
        self.hitch_angle_dial = QDial()
        self.hitch_angle_value = QLabel("Pot: 0.0 deg")
        self.hitch_vision_dial = QDial()
        self.hitch_vision_value = QLabel("Vision: --.- deg")

        self.hitch_angle_dial.setRange(-180, 180)
        self.hitch_angle_dial.setNotchesVisible(True)
        self.hitch_angle_dial.setWrapping(False)
        self.hitch_angle_dial.setEnabled(False)
        self.hitch_angle_dial.setValue(0)
        self.hitch_vision_dial.setRange(-180, 180)
        self.hitch_vision_dial.setNotchesVisible(True)
        self.hitch_vision_dial.setWrapping(False)
        self.hitch_vision_dial.setEnabled(False)
        self.hitch_vision_dial.setValue(0)
        self.calibrate_btn = QPushButton("Calibration")
        self.manual_rpm_value = QLabel("0.0")
        self.manual_speed_value = QLabel("0.00")
        self.manual_distance_value = QLabel("0.00")
    ...
    def _build_ui(self) -> None:
        ...
        hitch_dials = QHBoxLayout()
        pot_col = QVBoxLayout()
        pot_col.addWidget(self.hitch_angle_dial, alignment=Qt.AlignmentFlag.AlignHCenter)
        pot_col.addWidget(self.hitch_angle_value, alignment=Qt.AlignmentFlag.AlignHCenter)
        pot_col.addWidget(self.calibrate_btn, alignment=Qt.AlignmentFlag.AlignHCenter)
        vis_col = QVBoxLayout()
        vis_col.addWidget(self.hitch_vision_dial, alignment=Qt.AlignmentFlag.AlignHCenter)
        vis_col.addWidget(self.hitch_vision_value, alignment=Qt.AlignmentFlag.AlignHCenter)
        hitch_dials.addLayout(pot_col)
        hitch_dials.addLayout(vis_col)
        hitch_layout.addLayout(hitch_dials)
        ...
        manual_telemetry_group = QGroupBox("Manual Telemetry")
        manual_telemetry_form = QFormLayout(manual_telemetry_group)
        manual_telemetry_form.addRow("Speed (RPM)", self.manual_rpm_value)
        manual_telemetry_form.addRow("Speed (cm/s)", self.manual_speed_value)
        manual_telemetry_form.addRow("Distance (cm)", self.manual_distance_value)
        ...
        # Automatic view keeps telemetry only; camera panel is shown in the right column.
    ...
    def _build_auto_mpc(self) -> TruckTrailerMPC:
        # Straight up in +y is +pi/2 in this world frame.
    ...
    def _auto_tick(self) -> tuple[float, float]:
        ...
        if float(self.auto_q[4]) <= cfg.v_min + 1e-6:
            # Avoid infeasible warm-starts when we're already at min velocity.
        ...
        if u_opt is None:
            # Retry once from a neutral control guess to recover from bad warm-starts.
            ...
            if u_opt is None:
                ...
                # Keep moving safely even if one optimization step fails.
    ...
    def _update_hitch_angle_display(self, angle_deg: float) -> None:
        ...
        self.hitch_angle_dial.setValue(int(round(clamped)))
        self.hitch_angle_value.setText(f"Pot: {clamped:.1f} deg")
    ...
    def _update_hitch_vision_display(self, angle_deg: float | None) -> None:
        ...
        if angle_deg is None:
            self.hitch_vision_dial.setValue(0)
            self.hitch_vision_value.setText("Vision: --.- deg")
            return
        clamped = max(-180.0, min(180.0, float(angle_deg)))
        self.hitch_vision_dial.setValue(int(round(clamped)))
        self.hitch_vision_value.setText(f"Vision: {clamped:.1f} deg")
    ...
    def _draw_vehicle_box(
        ...
        # Build box in image-space from detected marker size and orientation,
        # so overlays remain visible even without homography calibration loaded.
        ...
        front = (half_marker + float(front_extra_cm)) * px_per_cm
        rear = (half_marker + float(rear_extra_cm)) * px_per_cm
        half_width = (half_marker + float(side_extra_cm)) * px_per_cm
    ...
    def _draw_truck_pivot_x(self, frame, corners: np.ndarray) -> None:
        center_px = corners.mean(axis=0).astype(float)
        top_mid = ((corners[0] + corners[1]) / 2.0).astype(float)
        fwd = top_mid - center_px
        norm = float(np.linalg.norm(fwd))
        ...
        # 4 cm behind bottom edge => half marker + 4 cm behind marker center.
        ...
        # High-contrast thick X: black underlay + bright yellow top.
    ...
    def _draw_workspace_box_from_reference_markers(
        self,
        frame,
        ...
        # Requested corners:
        # - top-left marker(10): bottom-right -> [2]
        # - top-right marker(11): bottom-left -> [3]
        # - bottom-right marker(12): top-left -> [0]
        # - bottom-left marker(13): top-right -> [1]
        ...
        if self.camera_H is not None and self.camera_H_inv is not None:
            world_poly = cv2.perspectiveTransform(poly.reshape(1, -1, 2), self.camera_H)[0]
            inset_side_cm = WORKSPACE_BOX_SIDE_INSET_CM
            extend_vertical_cm = WORKSPACE_BOX_VERTICAL_EXTEND_CM

            # Inset each corner by 2 cm along both adjoining edges.
            # Corner order: [top-left, top-right, bottom-right, bottom-left]
            w_tl, w_tr, w_br, w_bl = world_poly
            in_tl = w_tl + inset_side_cm * _unit(w_tr - w_tl) - extend_vertical_cm * _unit(w_bl - w_tl)
            in_tr = w_tr + inset_side_cm * _unit(w_tl - w_tr) - extend_vertical_cm * _unit(w_br - w_tr)
            in_br = w_br + inset_side_cm * _unit(w_bl - w_br) - extend_vertical_cm * _unit(w_tr - w_br)
            in_bl = w_bl + inset_side_cm * _unit(w_br - w_bl) - extend_vertical_cm * _unit(w_tl - w_bl)
            inset_world_poly = np.array([in_tl, in_tr, in_br, in_bl], dtype=np.float32)
            poly = cv2.perspectiveTransform(inset_world_poly.reshape(1, -1, 2), self.camera_H_inv)[0]
        else:
            # Fallback: estimate px/cm from detected reference marker side lengths.
        ...
        overlay = frame.copy()
        cv2.fillPoly(overlay, [poly_int], (255, 200, 40))
    ...
    def _apply_camera_focus_lock(self, cap) -> None:
        # Best-effort autofocus disable for cross-platform OpenCV backends.
        try:
            cap.set(cv2.CAP_PROP_AUTOFOCUS, 0)
        except Exception:
            pass
        # Some cameras expose a manual focus register. Use mid-range as default.
        try:
            ...
            if ids is not None:
                ...
                for marker_id in required_ref_ids:
                    ...
                    if corners is not None:
                        # Camera is stationary; retain latest seen reference marker corners.
                    ...
                    try:
                        if self.camera_H is not None and marker_pose_world is not None:
                            ...
                            if axis_tr is not None and axis_br is not None:
                                ...
                                origin = axis_pts_world[0].astype(float)  # (0,0) at bottom-right center
                            ...
                            # Fallback when homography is unavailable:
                            # derive a local cm frame from marker geometry in image space.
                            axis_tr = box_id_to_corners.get(11)
                            axis_br = box_id_to_corners.get(12)
                                ...
                                if not px_lengths:
                                    ...
                                    if px_per_cm <= 1e-6:
                                        ...
                                        if y_norm <= 1e-6:
                                            ...
                                            # Box center from inset polygon (pixel equivalent of 2 cm inset).
    ...
    def _draw_auto_prediction_path(self, frame) -> None:
        ...
        px_per_cm = float(self.auto_axis_px_per_cm)

        path_px: list[np.ndarray] = []
        for p in self.auto_pred_path_xy_cm:
            path_px.append(origin + x_hat * (float(p[0]) * px_per_cm) + y_hat * (float(p[1]) * px_per_cm))
