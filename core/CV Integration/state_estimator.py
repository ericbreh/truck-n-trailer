...
from vision.detect import load_calibration, marker_pose_world
    ...
    def open(self) -> None:
        ...
        if self._K is None:
            raise RuntimeError(
                ...
                "Run: python -m vision.calibrate_camera"
            )
        if self._H is None:
            raise RuntimeError(
                "Homography calibration not found. "
                "Run: python -m vision.calibrate_homography"
            )
